...
CGameScene::CGameScene(int stage)
	...
	addonList(), player(new CPlayer(D3DXVECTOR2(0, 0))), playerLife(5), rebirthTime(2), rebirthTimer(0), stage(stage), map(new CMap(stage)), timer(0), 
	isMoveTimer(0), tempTimer(0), count(0), isWin(false), isShakingCamera(false), shakeTime(0.3f), shakeAmount(20), shakePosition(0,0)
{
	gCamera = CMatrix();
...
void CGameScene::Update(float deltaTime)
{
	...
	if (player)
	{
		...
		gCamera.position.x = player->position.x - 384;
		if (gCamera.position.x < -64) gCamera.position.x = -64;
		if (gCamera.position.x > 64) gCamera.position.x = 64;

		if (player->isDie)
		{
			// 사망 이펙트
			for (int i = 0; i < player->attackPower; ++i)
			{
...
void CGameScene::classicUpdate(float deltaTime)
{
	...
	if (isShakingCamera)
	{
		...
		gCamera.position -= shakePosition;

		shakePosition.x = (getRandomNumber(0,shakeAmount)) - (shakeAmount / 2);
		shakePosition.y = (getRandomNumber(0, shakeAmount)) - (shakeAmount / 2);
