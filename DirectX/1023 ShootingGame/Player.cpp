...
CPlayer::CPlayer(D3DXVECTOR2 pos)
	:CMatrix(), attackTimer(0), attackTime(0), isAttack(false), attackPower(0), isDie(false), 
	isInvincible(false), invincible(false), invincibleTime(0), invincibleTimer(0), angle(0), isMove(false),
	addon(NULL), specialTime(0.01f), specialTimer(0), isSpecial(false), count(0), life(5)
{
	position = pos;
