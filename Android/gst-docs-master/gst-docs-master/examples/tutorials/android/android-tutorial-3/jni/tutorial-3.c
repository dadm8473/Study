...
app_function (void *userdata)
{
  ...
  /* Build pipeline */
  data->pipeline =
          gst_parse_launch ("ahcsrc device=0 ! video/x-raw,  width=320, heigh=500, framerate=30/1 ! videobalance contrast=1.3 brightness=0.3 saturation=1 hue=0 ! videoflip method=clockwise ! queue ! autovideosink sync=false \
                            alsasrc device=hw:1 ! audioconvert ! autoaudiosink",
      &error);

  // ahcsrc device=0 ! video/x-raw,  width=320, heigh=500 ! videoflip method=clockwise ! queue ! autovideosink sync=false\
  // audiotestsrc ! audioconvert ! autoaudiosink

  // directsoundsrc device-index=1 ! mulawenc ! rtppcmupay
